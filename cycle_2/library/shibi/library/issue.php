...
<center><input type="submit" name="b1" id="b1"  value="issue" /></center>
</form>
</fieldset>
</div>
<div id="d3">
...
while($v=mysqli_fetch_array($v1,MYSQLI_NUM))
{
	...
	{
?>
		<script> alert("THE BOOK IS NOT AVAILABLE");</script>
<?php
	}
	else
	{
	...
	mysqli_query($con,"INSERT INTO `issued_book`(`bk_id`, `stdnt_id`, `Issue_dt`, `Rnw_dt`) VALUES ('$bd','$sd','$isd','$rd')");
	mysqli_query($con,"UPDATE `book_details` SET `avilable`=0 WHERE `book_id`='$bd'");
...
?>	
	...
	<?php
	$v1=mysqli_query($con,"select * from issued_book where bk_id = '$bd'");
	while($v=mysqli_fetch_array($v1,MYSQLI_NUM))
        {
?>
 <tr>
